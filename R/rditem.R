...
btn_preview_RditemServer <- function(input,output,session,dms_token) {
  ...
  shiny::observeEvent(input$btn_preview_Rditem,{
    ...
    if(!is.null(var_file_expInfo_Rditem())){
...
btn_Update_RditemServer <- function(input,output,session,dms_token) {
  ...
  shiny::observeEvent(input$'btn_Update_Rditem',{
    if(!is.null(var_file_expInfo_Rditem())){
      ...
      data=tsdo::na_standard(data)
      #上传至数据库至重分类暂存表
      tsda::db_writeTable2(token = '9ADDE293-1DC6-4EBC-B8A7-1E5CC26C1F6C',table_name = 'rds_hrv_src_md_rditem_input',r_object = data,append = TRUE)
      #删除重分类已有数据
...
btn_download_RditemServer <- function(input,output,session,dms_token) {
  ...
  shiny::observeEvent(input$btn_view_Rditem,{
    ...
    tsui::run_dataTable2(id = 'mdlJHmd_Rditem_resultView',data =data_Rditem )
    
    
    #下载数据
    tsui::run_download_xlsx(id = 'mdlJHmd_Rditem_resultView',data =data_Rditem ,filename = '研发项目对照数据.xlsx')
